...
from backend.domain.ports.circuit_runner import DVCircuitRunner
...
@dataclass
class RunDVExperimentResponse:
    experiment: SingleExperiment


class RunDVExperimentUseCase:

    def __init__(
        self,
        runner: DVCircuitRunner,
    ...
    async def execute(
        self,
        request: RunDVExperimentRequest,
    ) -> RunDVExperimentResponse:

        hamiltonian = self._builder.build(request.model)

        if not await self._runner.is_available():
            raise RuntimeError(
                f"Backend '{self._runner.backend_name}' aint available."
            )
        ...
        return RunDVExperimentResponse(experiment=experiment)